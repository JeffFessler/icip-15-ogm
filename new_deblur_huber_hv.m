...
if 0
	...
	delta = 1e-2; %%% todo
	R = Reg1(mask, 'offsets', '2d:hv', 'pot_arg', {'huber', delta});
	...
	Ld = 1; % todo
	Lr = 8; %% max(col(R.sqs1_denom(R,xtrue)))
	L = Ld + lam*Lr; 
	...
	save('mat/deblur_huber_hv.mat'); 
	return;
else
	load('mat/deblur_huber_hv.mat'); 
end
...
for i=1:Niter
	...
	Xgm(:,i) = xcurr(:);
	Cgm(i) = norm(Fgrad(xcurr)); %F(xcurr);	
...
for i=1:Niter
	...
	ycurr = xcurr + (ti - 1)/tip*(xcurr - xprev); 
        ...
        Xfgm(:,i) = xcurr(:);
        Cfgm(i) = norm(Fgrad(xcurr)); %F(ycurr);       
...
for i=1:Niter
        ...
        Xogm(:,i) = xcurr(:);
        Cogm(i) = norm(Fgrad(xcurr)); %F(xcurr);         
...
% ogmog
Xogmog = zeros(numel(xtrue), Niter);
Cogmog = zeros(Niter,1);
...
for i=1:Niter
	...
	ycurr = xcurr + (Ti - ti)*tip/(ti*Tip)*(xcurr - xprev) ...
			+ (2*ti^2-Ti)*tip/(ti*Tip)*(xcurr - yprev);

        Xogmog(:,i) = ycurr(:);
        Cogmog(i) = norm(Fgrad(ycurr)); %F(ycurr);
...
%print('-depsc', 'fig/deblur_huber_hv_image.eps');

figure(2);
Iter = 1:Niter;
f2 = semilogy(Iter, Cgm, 'k-.', ...
	Iter, Cfgm, 'b--', ...
	Iter, Cogm, 'r-', ...
	Iter, Cogmog, 'm-.', ...
	Iter, repmat(Cfgm(end), [Niter 1]) - Fconv, 'k:');
axis([0 Niter 0 max(Cgm)]);
l2 = {'GM', 'FGM', 'OGM', 'OGM-OG'};
legend(f2, l2, 'fontsize', 25);
set(gca, 'fontsize', 22);
set(f2, 'markersize', 9, 'linewidth', 3);
print('-depsc', 'fig/deblur_huber_hv_grad_plot.eps');
